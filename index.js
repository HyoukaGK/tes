...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝐘𝐀𝐄 𝐌𝐚𝐧𝐨 @${num.split('@')[0]}\nBem vindo (𝐀) ao grupo *${mdata.subject}* leia as regras pra nn ser removido Karay, e mame os adms, membro comum vem depois`
				...
				teks = `Vai pela sombra, Ghost nao faz falta😑 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'CALMA TENHA CALMA TO FAZENDO AQUI. Se demorar muito da o comando de novo, mas sem spam.',
				success: '✔️Sucesso✔️',
				...
				error: {
					stick: '[❗] Deu errado mano, tenta concertar ai  ❄',
					Iv: '❄ Link inválido ❄'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❄',
					ownerG: '[❗] Este comando só pode ser usado pelo grupo proprietário! ❄',
					ownerB: '[❗] Este comando só pode ser usado pelo K1R1T0 e pelo Daniel! ❄',
					admin: '[❗] Este comando so pode ser usado por adm membro comum! ❄',
					Badmin: '[❗] Me da adm pra esse comando funcionar ;-;! ❄',
